...
node {
    ...
    stage('SNAPSHOT Maven Build'){
        ...
        rtMaven.resolver server: artiServer, snapshotRepo: 'maven-virtual-test-outer', releaseRepo: 'libs-release'
        rtMaven.deployer server: artiServer, snapshotRepo: 'libs-snapshot', releaseRepo: 'libs-stage-local'
        rtMaven.deployer.deployArtifacts = false

        rtMaven.tool = 'maven'
        rtMaven.run pom: 'guestbook-service/pom.xml', goals: 'clean install', buildInfo: buildInfo
    }
    ...
    stage('Release Maven Build'){
        ...
        rtMaven.resolver server: artiServer, snapshotRepo: 'libs-snapshot', releaseRepo: 'libs-stage-local'
        rtMaven.deployer server: artiServer, snapshotRepo: 'libs-snapshot', releaseRepo: 'libs-stage-local'
        rtMaven.deployer.deployArtifacts = false
